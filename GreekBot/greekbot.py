...
class GreekBot(object):
    ...
    def __init__(self, name, **kwargs):
        ...
        #using the greek SQL Storage Adapter
        storage_adapter = kwargs.get('storage_adapter', 'GreekBot.greek_sql_storage.GreekSQLStorageAdapter')

        #using the greek Best Match logic adapter
        logic_adapters = kwargs.get('logic_adapters', [
            'GreekBot.greek_best_match.BestMatch'
        ])
    ...
    def neaKataxwrhsh(self,protash,prohgoumenh,weightToAdd):
        '''
        Saves a new response to a sentence by adding the statement to the database and then updating its weight

        :param protash: the text of the response
        :param prohgoumenh: the text of the previous sentence
        :param weightToAdd: weight to be added to the default weight of 1, can be negative
        '''

        #creating a statement out of the sentences
        toBeSaved = Statement(protash, prohgoumenh)
        toBeSaved.search_text = self.storage.tagger.get_bigram_pair_string(protash)
        toBeSaved.search_in_response_to = self.storage.tagger.get_bigram_pair_string(prohgoumenh)
        self.learn_response(toBeSaved)
        self.storage.updateWeight(protash, prohgoumenh, weightToAdd)
    ...
    def get_response(self, statement=None, **kwargs):
        ...
        Doesn't save the response or the input to the database at this stage.
    ...
    def generate_response(self, input_statement, additional_response_selection_parameters=None):
        ...
        for adapter in self.logic_adapters:
            if adapter.can_process(input_statement):
                ...
                print(str(output.id)+": "+str(output.confidence)+" * "+str( self.storage.getWeightFromId(output.id)))

                if output.confidence*self.storage.getWeightFromId(output.id) > max_confidence:
                    result = output

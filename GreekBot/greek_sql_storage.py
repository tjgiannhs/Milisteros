...
class GreekSQLStorageAdapter(StorageAdapter):
    """
    ...
    #this is the default reply, to be used by other files
    defaultReply = "Συγνώμη, δε σας κατάλαβα. Παρακαλώ επαναδιατυπώστε την ερώτησή σας."
    ...
    def get_statement_model(self):
        ...
        """
        from GreekBot.greek_models import Statement
        return Statement
    ...
    def get_tag_model(self):
        ...
        """
        from GreekBot.greek_models import Tag
        return Tag
    ...
    def updateWeight(self,protash,prohgoumenh,baros):
        '''
        We add the value of baros to the weight of the statement in the database with text
        and in_response_to the values of protash and prohgoumenh

        :param protash: the text of the response
        :param prohgoumenh: the text of the previous sentence
        :param baros: weight to be added, can be negative
        '''

        Statement = self.get_model('statement')

        session = self.Session()
        old = session.execute('SELECT weight FROM statement WHERE text="'+protash+'" AND in_response_to="'+prohgoumenh+'"').fetchall()

        #we want the first and only statement it returns, we use it to get the float value of the weight
        for timh in old[0]:
            oldBaros = float(timh)
            break

        newBaros = baros+oldBaros
        #clamp newBaros' value between 0 and 2
        newBaros = max(min(newBaros, 2.0), 0.0)

        session.execute('UPDATE statement SET weight="'+str(newBaros)+'" WHERE text="'+protash+'" AND in_response_to="'+prohgoumenh+'"')
        session.commit()  #in order to save the changes to the database
        session.close()


    def updateWeightFromId(self,id,baros):
        '''
        We add the value of baros to the weight of the statement in the database with id equal to the id given

        :param id: the id of the statement
        :param baros: the weight to be added, float, can be negative
        '''

        Statement = self.get_model('statement')

        session = self.Session()
        old = session.execute('SELECT weight FROM statement WHERE id="' + str(id) + '"').fetchall()
        ...
        session.execute('UPDATE statement SET weight="' + str(newBaros) + '" WHERE id="' + str(id) + '"')
        session.commit()  #in order to save the changes to the database
        session.close()



    def getWeightFromId(self,ari8mos):
        '''
        Returns the weight of a sentence using its id in the database

        :param ari8mos: the id of the sentence, int
        ...
        session = self.Session()
        kataxwrhsh = session.execute('SELECT weight FROM statement WHERE id=' + str(ari8mos) + '').fetchall()

        #we want the first and only statement it returns, we use it to get the float value of the weight
        for timh in kataxwrhsh[0]:
            baros = float(timh)
            break
        session.close()
        return baros

    def isAlreadyStored(self, protash, prohgoumenh):
        '''
        Returns True or False if protash is already stored in the databse as a response to prohgoumenh

        :param protash: a String which is in reply to prohgoumenh
        :param prohgoumenh: a String
        ...
        Statement = self.get_model('statement')
        session = self.Session()
        findingsAmount = session.query("* FROM statement WHERE text='"+protash+"' AND in_response_to='"+prohgoumenh+"'").count()
        print("Bre8hkan omoia:"+str(findingsAmount))
        if findingsAmount>0:
            alreadyStored = True
        session.close()
    ...
    def filter(self, **kwargs):
        ...
        '''print("Total statements filtered: ", total_statements)
        for s in statements:
            print(s.text)
        '''
    ...
    def create_database(self):
        ...
        """
        from GreekBot.greek_models import Base
